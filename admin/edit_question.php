...
try {
    ...
    error_log("Failed loading question details: " . $e->getMessage());
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($question_text) || empty($option_a) || empty($option_b) || empty($option_c) || empty($option_d)) {
        $error = "All options and question description are required.";
        ...
        try {
            $upStmt = $pdo->prepare("
                UPDATE questions 
                SET question_text = ?, option_a = ?, option_b = ?, option_c = ?, option_d = ?, correct_answer = ?, explanation = ?, marks = ?, status = ? 
                WHERE id = ?
            ");
            $upStmt->execute([$question_text, $option_a, $option_b, $option_c, $option_d, $correct_answer, $explanation, $marks, $status, $question_id]);
            
            $_SESSION['success_message'] = "Question updated successfully.";
            header("Location: questions.php");
            exit();
        } catch (PDOException $e) {
            error_log("Failed updating question record: " . $e->getMessage());
            $error = "Database error. Could not update question.";
        }
    }
}
...
<div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-4 border-bottom">
    <h1 class="h2 fw-bold text-dark"><i class="bi bi-pencil text-primary me-2"></i>Edit Question Details</h1>
    ...
    <div class="card-body p-4">
        <form action="edit_question.php?id=<?php echo $question_id; ?>" method="POST">
            ...
            <div class="mb-4">
                ...
                <textarea class="form-control" id="question_text" name="question_text" rows="4" required><?php echo escape($question['question_text']); ?></textarea>
            </div>
            ...
            <div class="row g-3 mb-4">
                <div class="col-md-6">
                    ...
                    <input type="text" class="form-control" id="option_a" name="option_a" value="<?php echo escape($question['option_a']); ?>" required>
                </div>
                <div class="col-md-6">
                    <label for="option_b" class="form-label fw-semibold text-primary">Option B</label>
                    <input type="text" class="form-control" id="option_b" name="option_b" value="<?php echo escape($question['option_b']); ?>" required>
                </div>
                <div class="col-md-6">
                    <label for="option_c" class="form-label fw-semibold text-primary">Option C</label>
                    <input type="text" class="form-control" id="option_c" name="option_c" value="<?php echo escape($question['option_c']); ?>" required>
                </div>
                <div class="col-md-6">
                    <label for="option_d" class="form-label fw-semibold text-primary">Option D</label>
                    <input type="text" class="form-control" id="option_d" name="option_d" value="<?php echo escape($question['option_d']); ?>" required>
                </div>
            </div>
            ...
            <div class="row g-3 mb-4 pt-3 border-top">
                ...
                <div class="col-md-4">
                    ...
                    <input type="number" class="form-control" id="marks" name="marks" min="1" value="<?php echo (int)$question['marks']; ?>" required>
                </div>
                ...
                <div class="col-md-4">
                    ...
                    <select class="form-select" id="status" name="status">
                        <option value="active" <?php echo ($question['status'] === 'active') ? 'selected' : ''; ?>>Active</option>
                        <option value="inactive" <?php echo ($question['status'] === 'inactive') ? 'selected' : ''; ?>>Inactive</option>
                    </select>
                </div>
            </div>
            ...
            <div class="mb-4 pt-3 border-top">
                ...
                <textarea class="form-control" id="explanation" name="explanation" rows="3"><?php echo escape($question['explanation']); ?></textarea>
            </div>

            <div class="d-flex gap-2 pt-3 border-top">
                <button type="submit" class="btn btn-primary px-4 py-2"><i class="bi bi-save me-1"></i> Update Details</button>
