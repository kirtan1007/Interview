...
> [!IMPORTANT]
> **DO NOT** copy-paste this entire `.md` file into phpMyAdmin's SQL query tab, as the markdown descriptions will cause SQL syntax errors.
> Instead, please do one of the following:
> 1. Copy **only** the SQL code blocks from this file.
> 2. **(Recommended)** Directly import or copy the contents of the raw SQL file: [interview.sql](file:///c:/Users/kpanc/OneDrive/Desktop/Interview/interview.sql) (also located in `interview-system/interview.sql` under your XAMPP server).

This file contains the complete MySQL database setup query documentation. The SQL inside this document is fully compatible with **phpMyAdmin** and standard MySQL instances.

---

## 1. Create Database and Setup Environment
...
Stores both Admin and Student accounts. Students cannot register themselves; they are created by the Admin.
...
Stores the MCQ questions created by the Admin.
...
Tracks student test attempts. By adding a `UNIQUE` index/constraint on `student_id`, we enforce the crucial business rule that **one student can have at most one attempt**.
...
## 5. Create Student Answers Table

Stores the detailed responses of students for each question during their attempt.
...
## 5.5. Create Security Violations Table

Stores proctoring violations and anti-cheating events flagged during candidate examinations.
...
Stores global configurations that Admins can modify dynamically.
...
Inserts a default Admin and a default Student. The passwords are securely pre-hashed using `password_hash()` via `PASSWORD_BCRYPT`.
